...
#### 「clone」
cloneコマンドは指定されたディレクトリに元のリポジトリと同じものを複製するコマンドである。Githubからソースをコピーしたいときなどに使う。
#### 「commit」
commitコマンドは追加・変更したファイルをGitに反映させるためのコマンドである。通常のファイル操作では上書き保存でファイルの中身が変更されるが、Gitのリポジトリに変更を登録するにはcommitコマンドを使う必要がある。
#### 「push」
pushコマンドはリモートリポジトリのブランチ履歴を更新するためのコマンドである。ローカル環境にあるブランチ上で作成されたコミット履歴と関連連するブランチ名などを任意のリモートレポジトリに送信することで、そのリモート上でのコミット履歴を更新できる。
#### 「branch」
branchコマンドは、ブランチの作成、一覧表示、名称変更、削除を行うコマンドである。このコマンドにはブランチの切り替えを行う機能も、分岐した履歴を統合して元に戻す機能もない。そのため、多くの場合branchコマンドはcheckoutコマンドおよびmergeコマンドと併用される。
#### 「fetch」
fetchコマンドは、リモートリポジトリからローカルリポジトリにコミット、ファイル、参照をダウンロードするコマンドである。フェッチとは、他の開発者の作業内容を確認する場合に行う操作である。
#### 「pull」
pullコマンドは、リモートリポジトリからコンテンツをフェッチしてダウンロードし、そのコンテンツと一致するようローカルリポジトリを即時に更新するために使用される。pullコマンドは、実際には他の2つのコマンド (git fetch とその後の git merge) を組み合わせたものである。
#### 「merge」
mergeコマンドは、git branch を使用して作成された、独立した複数の開発ラインをひとつのブランチに統合するコマンドである。
#### 「rebase」
rebaseコマンドは、1つのブランチから別のブランチへの変更を統合することを専門とした、2つのGitユーティリティの1つである。もう1つの変更統合ユーティリティは git merge である。rebaseコマンドには強力な履歴書き換え機能を備えている。

### 3-3. プロジェクト進行に、Pull Requestを用いたGitHub Flowを採用することで、解決できるチーム開発での問題はなにか (実際にGitHub Flowで作業する手順も説明できる範囲でする)
GitHub Flow で作業するときは、初めに分岐を作成し、既定のブランチに影響を与えずに作業するスペースを作成する。次に、ブランチで、リポジトリに必要な変更を加える。フィードバックを求める準備ができるまで、引き続きブランチに変更を加え、コミットし、プッシュする。その次には、pull requestを作成し、レビューに対応する。pull requestが承認されたら、マージしてブランチを削除する。以上のように作業をすることによって、安全に変更を元に戻したり、マージする前にエラーを把握できるようになる。
...
- [Git Branch コマンド | Atlassian Git Tutorial](https://www.atlassian.com/ja/git/tutorials/using-branches)
- [git fetch | Atlassian Git Tutorial](https://www.atlassian.com/ja/git/tutorials/syncing/git-fetch)
- [git pull | Atlassian Git Tutorial](https://www.atlassian.com/ja/git/tutorials/syncing/git-pull)
- [Git merge | Atlassian Git Tutorial](https://www.atlassian.com/ja/git/tutorials/using-branches/git-merge)
